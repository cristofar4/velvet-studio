...
export default function Cursor() {
  ...
  return (
    <div ref={rootRef} style={{ display: "none" }} aria-hidden>
      <div
        ref={dotRef}
        className="pointer-events-none fixed left-0 top-0 z-[160] size-1.5 rounded-full bg-champagne transition-opacity duration-300"
        style={{ opacity: active && label ? 0 : 1 }}
      />
      <div
        ref={ringRef}
        className={`pointer-events-none fixed left-0 top-0 z-[159] flex items-center justify-center rounded-full border transition-[width,height,background-color,border-color,backdrop-filter] duration-300 ease-out ${
          label
            ? "size-20 border-champagne/50 bg-night/55 backdrop-blur-sm"
            : active
              ? "size-12 border-gold/80"
              : "size-8 border-silver/40"
        }`}
      >
        <span
          className={`eyebrow tracking-[0.18em]! text-[10px] text-champagne transition-opacity duration-200 ${
